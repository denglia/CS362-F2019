...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case minion:
        return playMinion(state,choice1);
    ...
    case ambassador:
        return playAmbassador(state, choice1);
...
int playMinion(struct gameState *state, int choice1){
  //+1 action
  state->numActions++;
...
int playAmbassador(struct gameState *state, int choice1){
...
int playMine(struct gameState *state, int choice1){
  j = state->hand[currentPlayer][choice1];  //store card we will trash
  ...
  //discard card from hand
  discardCard(handPo, currentPlayer, state, 0);
